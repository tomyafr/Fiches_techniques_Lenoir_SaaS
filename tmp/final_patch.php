<?php
$file = 'api/rapport_final.php';
$content = file_get_contents($file);

$badBlock = '                            p.querySelectorAll(\'input[type="radio"]:checked\').forEach(r => {
...
$goodBlock = '                            p.querySelectorAll(\'input[type="radio"]:checked\').forEach(r => {
                                const lbl = r.closest(\'label\');
                                if (lbl) lbl.classList.add(\'selected\');
                            });
...
if (strpos($content, $badBlock) !== false) {
    $newContent = str_replace($badBlock, $goodBlock, $content);
    file_put_contents($file, $newContent);
    echo "SUCCESS: Block replaced.";
} else {
    echo "ERROR: Bad block not found!";
}
?>
