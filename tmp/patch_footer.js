...
// 1. Update createPdfFooter
const oldFooterText = `        function createPdfFooter() {
...
const newFooterText = `        function createPdfFooter() {
            return document.createElement('div');
        }`;

if (content.indexOf(oldFooterText) !== -1) {
    content = content.replace(oldFooterText, newFooterText);
    console.log("createPdfFooter updated successfully.");
} else {
    console.log("createPdfFooter match not found.");
}

// 2. Update PDF options and footer drawing
const oldGenText = `            const opt = {
...
const newGenText = `            const opt = {
                margin: [10, 0, 25, 0], // Reservé pour le footer natif
                filename: window.LM_RAPPORT ? window.LM_RAPPORT.pdfFilename : 'rapport.pdf',
                image: { type: 'jpeg', quality: 0.98 },
                html2canvas: { scale: 2, useCORS: true, logging: false },
                jsPDF: { unit: 'mm', format: 'a4', orientation: 'portrait' },
...
if (content.indexOf(oldGenText) !== -1) {
    content = content.replace(oldGenText, newGenText);
    console.log("genererPDFBase64 options updated successfully.");
} else {
    console.log("genererPDFBase64 match not found.");
}
