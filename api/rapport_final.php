...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($_POST['action'] === 'save_rapport') {
        try {
            ...
            if (empty($contactPrenom)) { $error = "Le pr├®nom du contact est obligatoire."; }
            elseif (empty($contactNom)) { $error = "Le nom du contact est obligatoire."; }
            elseif (strlen($contactPrenom) > 50) { $error = "Le pr├®nom du contact ne doit pas d├®passer 50 caract├¿res."; }
...
<body>
    ...
    <script>
        ...
        // ══════════════════════════════════════════════════════════════════
        // MOTEUR PDF SÉQUENTIEL (jsPDF + html2canvas page-par-page)
        // Supporte 100+ pages. Chaque page est rendue séparément pour
        // libérer la mémoire entre les renders → plus de pages blanches.
        // ══════════════════════════════════════════════════════════════════
        async function _renderPdfEngine(isDownload) {
            const jsPDFLib = window.jspdf ? window.jspdf.jsPDF : (window.jsPDF);
            if (!jsPDFLib) throw new Error('jsPDF non disponible. html2pdf.js doit être chargé.');
            if (!window.html2canvas) throw new Error('html2canvas non disponible.');

            const container = await buildFullPdfContainer();
            await ensureImagesBase64(container);

            // Le container doit être dans le DOM pour que html2canvas mesure correctement
            ...
            if (pages.length === 0) throw new Error('Aucune page PDF trouvée dans le container.');
            ...
            for (let i = 0; i < pages.length; i++) {
                ...
                const imgH_mm = (canvasH / canvasW) * PDF_W_MM;

                if (i > 0) pdf.addPage();

                // Si la page dépasse 297mm, la scaler pour tenir
                if (imgH_mm <= PDF_H_MM) {
                    pdf.addImage(imgData, 'JPEG', 0, 0, PDF_W_MM, imgH_mm, '', 'FAST');
                } else {
                    // Page très haute : la répartir sur plusieurs pages PDF
                    const pxPerMm = canvasW / PDF_W_MM;
                    const sliceH_px = Math.floor(PDF_H_MM * pxPerMm);
                    let offsetY = 0;
                    let firstSlice = true;
                    while (offsetY < canvasH) {
                        const remaining = canvasH - offsetY;
                        const sliceH = Math.min(sliceH_px, remaining);
                        const sliceCanvas = document.createElement('canvas');
                        sliceCanvas.width = canvasW;
                        sliceCanvas.height = sliceH;
                        sliceCanvas.getContext('2d').drawImage(canvas, 0, offsetY, canvasW, sliceH, 0, 0, canvasW, sliceH);
                        const sliceData = sliceCanvas.toDataURL('image/jpeg', 0.92);
                        const sliceH_mm = (sliceH / canvasW) * PDF_W_MM;
                        if (!firstSlice) pdf.addPage();
                        pdf.addImage(sliceData, 'JPEG', 0, 0, PDF_W_MM, sliceH_mm, '', 'FAST');
                        offsetY += sliceH;
                        firstSlice = false;
                        sliceCanvas.width = 1; sliceCanvas.height = 1; // Libérer mémoire
                    }
                }

                // Libérer la mémoire du canvas immédiatement
                canvas.width = 1;
                canvas.height = 1;
            }
            ...
            if (isDownload) {
                const filename = window.LM_RAPPORT ? window.LM_RAPPORT.pdfFilename : 'rapport.pdf';
                pdf.save(filename);
                return null;
            } else {
        ...
        async function genererPDFBase64() {
            return await _renderPdfEngine(false);
        }
