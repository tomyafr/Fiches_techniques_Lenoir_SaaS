                ...
                for (let mIdx = 0; mIdx < totalMachines; mIdx++) {
                    ...
                    try {
                        ...
                        pages.forEach((p, pIdx) => {
                            ...
                            // Gestion sélective de .no-print-pdf :
                            // - Les <img> avec cette classe = logos inutiles → ON LES SUPPRIME
                            // - Les <div> avec cette classe = diagrammes/schémas → ON RETIRE JUSTE LA CLASSE
                            p.querySelectorAll('.no-print-pdf').forEach(el => {
                                if (el.tagName === 'IMG' || el.tagName === 'P') {
                                    el.remove(); // Logos et messages texte
                                } else {
                                    el.classList.remove('no-print-pdf'); // Diagrammes : garder le contenu
                                }
                            });
